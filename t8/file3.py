...
mtx = cv.getRotationMatrix2D((windows.shape[1] / 2,
                              windows.shape[0] / 2), 210, 0.6)
windows210 = cv.warpAffine(windows, mtx, (windows.shape[1], windows.shape[0]))
findMatches(windows210, 'b')

mtx = cv.getRotationMatrix2D((windows.shape[1] / 2,
                              windows.shape[0] / 2), 0, 2)
windows2x = cv.warpAffine(windows, mtx, (windows.shape[1], windows.shape[0]))
findMatches(windows2x, 'c')

mtx = cv.getRotationMatrix2D((windows.shape[1] / 2,
                              windows.shape[0] / 2), 0, .5)
windows05x = cv.warpAffine(windows, mtx, (windows.shape[1],
                                          windows.shape[0]))
findMatches(windows05x, 'd')

mtx = cv.getRotationMatrix2D((windows.shape[1] / 2,
                              windows.shape[0] / 2), 270, 1.5)
windows27005x = cv.warpAffine(windows, mtx,
